...
1. Codex CLI に「`example.txt` を読み、その内容で `solve_linear_program` を実行してください」と指示します。Codex はファイル内容を解析し、変数の下限制約・目的関数の向きと係数・制約行を JSON にまとめてツールへ送信します。`example.txt` のフォーマットは特に固定していないため、CLI 側が解析できる形であれば自由に記述できます。
